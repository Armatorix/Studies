...
int main()
{
	...
	initCrypter();
	mpz_set_ui(privatekey,415595);
	mpz_set_ui(publickey,707);
	mpz_set_ui(module,454099);
	//generateKeys();
	...
	mpz_init(temp);
	mpz_init(mess);
	mpz_set_ui(mess,66666666666);
	//gmp_printf("Message:\n%Zd\n", mess);
	cryptMessage(crypted,mess);
	mpz_t total;
	mpz_init(total);
	
	int rounds = 1000000;
	unsigned long long mX=100000;
	clock_t start,end;
	/*
	mpz_set_ui(total,0);
	for (int i=0;i<rounds;i++)
	{
		mpz_set_ui(temp,0);
		start = clock();
		mpz_mul(temp,publickey,publickey);
		end = clock();
		elapsedTime = mX*abs(end-start);   
		mpz_add_ui(total,total,elapsedTime);
		printf("\r%d%%",i*100/rounds);
	}
	printf("\r");
	mpz_div_ui(total,total,rounds);
	gmp_printf("Mult:%Zd\n",total);
	...
	*/
	...
	for (int i=0;i<rounds;i++)
	{
		...
		mpz_sub_ui(temp,privatekey,1);
		mpz_mul(temp,mess,privatekey);
		mpz_mod(temp,temp,module);
		mpz_sub_ui(temp,privatekey,1);
		mpz_mul(temp,mess,privatekey);
		mpz_mod(temp,temp,module);
		mpz_sub_ui(temp,privatekey,1);
	...
	printf("\r");
	mpz_div_ui(total,total,20*rounds);
	gmp_printf("If 1:%Zd\n",total);
	mpz_set_ui(total,0);
	...
	for (int i=0;i<rounds;i++)
	{
		...
		mpz_div_ui(temp,privatekey,2);
		mpz_mul(temp,mess,mess);
		mpz_mod(temp,temp,module);
		mpz_div_ui(temp,privatekey,2);
		mpz_mul(temp,mess,mess);
		mpz_mod(temp,temp,module);
	...
	printf("\r");
	mpz_div_ui(total,total,rounds*20);
	gmp_printf("If 0:%Zd\n",total);
	mpz_set_ui(total,0);
